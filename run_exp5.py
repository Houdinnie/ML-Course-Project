...
results = {}
for aug_name, gen in [('none', None), ('weak', ImageDataGenerator(rotation_range=15, horizontal_flip=True))]:
    print(f'>>> {aug_name}', flush=True)
    m = Sequential([Input((64,64,3)),
                    Conv2D(32,(3,3),padding='same',activation='relu'),MaxPooling2D((2,2)),BatchNormalization(),Dropout(.3),
                    Conv2D(32,(3,3),padding='same',activation='relu'),MaxPooling2D((2,2)),BatchNormalization(),Dropout(.3),
                    Flatten(),Dense(256,activation='relu'),Dropout(.3),Dense(2,activation='softmax')])
    m.compile(optimizer=Adam(.001),loss='categorical_crossentropy',metrics=['accuracy'])
    if gen:
        m.fit(gen.flow(X_tr,y_tr,batch_size=32),epochs=5,validation_data=(X_te,y_te),verbose=2)
    else:
    ...
    print(f'  {aug_name}: {acc*100:.2f}%', flush=True)
    del m; gc.collect()

print('\n=== Exp 5 Results ===', flush=True)
for k,v in results.items(): print(f'  {k}: {v*100:.2f}%', flush=True)
print('Exp 5 DONE', flush=True)